...
TWO_BIT_TO_FA="/hpc/group/igvf/revathy/software/twoBitToFa"
TWO_BIT="/hpc/group/igvf/A549/extra_GCs/hg38.2bit"
CHILD="/hpc/group/igvf/revathy/BlueSTARR/mutator-child-TE.py"
...
#     #     pair = reader.nextSequence()
#     #     if pair is None:
#     #         print("Warning: fewer FASTA sequences than CREs!")
#     #         break
#         (defline,seq)=pair
...
#################################################################################################
### RunModel changed to use a temporary input file for each batch before calling mutator-child.py
### This is to prevent overwriting before the subprocess finishes reading it
#################################################################################################
        ...
        for Pos in positions:
            ...
            for allele in alleles:
                ...
                if seq[local] != ref:
                    raise Exception("Ref mismatch: " + seq[local] + " vs " + ref)
                altSeq = seq[:local] + allele + seq[(local + 1):]
                print(cre.ID + "\t" + actualInterval + "\tpos=" + str(pos) +
                      "\tref=" + ref + "\t" + allele + "\t" + altSeq, file=INPUTS)
                n += 1

                if n >= JOBSIZE:
                    INPUTS.close()

                    # Create a named temp file on disk
                    with tempfile.NamedTemporaryFile(delete=False, mode="w", suffix=".inputs") as temp_input:
                        temp_input_name = temp_input.name
                        with open(inputFile, "r") as src:
                            temp_input.write(src.read())

                    # Run child script using subprocess
                    cmd = f"{CHILD} {model} {temp_input_name}"
...
print(f"Loaded {len(recs)} CREs", flush=True)
...
num_fasta_seqs = count_fasta_entries(fastaFile)
print(f"FASTA has {num_fasta_seqs} sequences")

print("Running the model",flush=True)
print(recs,seqLen,fastaFile,inputFile,outputFile,JOBSIZE,model)
