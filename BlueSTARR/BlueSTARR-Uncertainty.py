...
def summary_statistics(X, Y, set, taskNum, numTasks, taskName, model, idx, modelFilestem):
    ...
    mu=pred[:,0]
    sigma=tf.nn.softplus(pred[:,1]).numpy()

    if (config.useCustomLoss) :
        naiveTheta, cor=naiveCorrelation(Y,mu,taskNum,numTasks) # naiveTheta: normal scale, pred: log scale
        df = pd.DataFrame({'idx':idx, 'true':tf.math.log(naiveTheta),'mu_pred': mu,'sigma':sigma}) # log scale
        corr_sigma = spearmanr(np.abs(df['mu_pred'] - df['true']),df['sigma'])
...
def naiveCorrelation(y_true, mu_pred, taskNum, numTasks):
    ...
    cor=None
    if(numTasks==1):
        cor=stats.spearmanr(tf.math.exp(mu_pred.squeeze()),naiveTheta)
    else:
        cor=stats.spearmanr(tf.math.exp(mu_pred[taskNum].squeeze()),naiveTheta)
...
def makeClosure(taskNum):
    ...
    def loss(y_true, y_pred):
        ...
        nll = 0.5*(tf.math.square((naiveTheta-mu)/sigma)+2.0*tf.math.log(sigma)+tf.math.log(two_pi)) 
...
def BuildModel(seqlen):
    ...
    for i in range(numTasks):
        ...
        sigma_head = kl.Dense(1, activation='linear', bias_initializer=Constant(-2.0), name=f"{task}_sigma")(x)

        out = kl.Concatenate(name=task)([mu_head, sigma_head])
        print(out)
        outputs.append(out)
