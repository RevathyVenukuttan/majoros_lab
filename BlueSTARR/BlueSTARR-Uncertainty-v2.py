...
def main(configFile,subdir,modelFilestem):
    ...
    # Build model
    model, mu_tensors=BuildModel(seqlen)
    model.summary()
...
def summary_statistics(X, Y, set, taskNum, numTasks, taskName, model, idx, modelFilestem):
    pred_list = model.predict(X, batch_size=config.BatchSize)
    # pred = pred_list if numTasks==1 else pred_list[taskNum]
    mu_pred = np.squeeze(pred_list[2*task_num])
    var_pred_raw = np.squeeze(pred_list[2*task_num + 1])
    var_pred = np.asarray(tf.nn.softplus(var_pred_raw) + 1e-6)

    if (config.useCustomLoss) :
        naiveTheta, cor, SE = naiveCorrelation(Y,mu,taskNum,numTasks) 
        df = pd.DataFrame({'idx':idx, 'true':y_true_log,'mu_pred': mu_pred,'SE':SE, 'var_pred':var_pred}) 
        ...
        df = pd.DataFrame({'idx':idx, 'true':Y.numpy().ravel(),'mu_pred': mu}) # log scale
        mse = np.mean((df['true'] - df['mu_pred'])**2)
        df.to_csv(modelFilestem+'.txt', index = False, sep='\t')
    
    print(taskName+" predicted theta rho=",cor.statistic,"p=",cor.pvalue)
    print("Correlation between error and sigma=",corr_sigma.statistic,"p=",corr_sigma.pvalue)
    print(taskName+' predicted theta mse=', mse)


    
def naiveCorrelation(y_true, mu_pred, taskNum, numTasks):
    a=0
    ...
    naiveTheta = (sumY / sumX).numpy()
    y_true_log = np.log(naiveTheta)

    SE = np.square(mu_pred - y_true_log)
    cor=None
    if(numTasks==1):
        cor=stats.spearmanr(tf.math.exp(mu_pred.squeeze()),y_true_log)
    else:
        cor=stats.spearmanr(tf.math.exp(mu_pred[taskNum].squeeze()),y_true_log)
    return naiveTheta, cor, SE
...
#         # log_naiveTheta=tf.math.log(naiveTheta)
#         mu=y_pred[:,0]
#         # print('Mu='); print(mu)
#         raw_sigma=y_pred[:,1]
#         # print('Raw_Sigma='); print(raw_sigma)
#         sigma=tf.nn.softplus(raw_sigma)+1e-6 ## avoid this if using ReLU for the sigma
#         # print("sigma="); print(sigma)
#         # lognormal_dist = tfd.LogNormal(loc=mu, scale=sigma)
#         # log_density = lognormal_dist.log_prob(tf.math.log(naiveTheta))
#         # nll=-log_density
#         two_pi = tf.cast(2.0*math.pi, tf.float32)
#         nll = 0.5*(tf.math.square((naiveTheta-mu)/sigma) + 2.0*tf.math.log(sigma) + tf.math.log(two_pi)) ## check the derivation 
#         ## add a negative sign to the nll
#         # print("NLL="); print(nll)
#         return nll
#     return loss
...
@tf.autograph.experimental.do_not_convert
def make_mu_loss(taskNum):
    # Computes L_mu = (mu - log(naiveTheta))^2
    a = 0
    for i in range(taskNum): a += NUM_DNA[i] + NUM_RNA[i]
    b = a + NUM_DNA[taskNum]
    c = b + NUM_RNA[taskNum]
    ...
    @tf.autograph.experimental.do_not_convert
    def loss(y_true, y_pred):
        # y_pred is the mu-head output with shape (batch, 1)
        DNA = y_true[:, a:b] + 1.0
        RNA = y_true[:, b:c] + 1.0
        sumX = tf.reduce_sum(DNA, axis=1)
        sumY = tf.reduce_sum(RNA, axis=1)
        naiveTheta = sumY / sumX
        y = tf.math.log(naiveTheta)                 # target on log-scale
        mu = tf.squeeze(y_pred, axis=-1)            # predicted log(theta)
        se = tf.square(mu - y)                      # squared error
        return se
    return loss


@tf.autograph.experimental.do_not_convert
def make_var_loss(taskNum, mu_tensor):
    # Computes L_var = (var - stop_gradient(SE))^2
    a = 0
    for i in range(taskNum): a += NUM_DNA[i] + NUM_RNA[i]
    b = a + NUM_DNA[taskNum]
    c = b + NUM_RNA[taskNum]
    ...
    def loss(y_true, y_pred):
        ...
        naiveTheta = sumY / sumX
        print(naiveTheta)
        y = tf.math.log(naiveTheta)
        print(y.shape)
        mu = tf.squeeze(mu_tensor, axis=-1)         # use the paired mu head
        print(mu.shape)
        se = tf.square(mu - y)                      # SE from the mu head

        # map raw -> variance with softplus to ensure positivity & stability
        raw_var = tf.squeeze(y_pred, axis=-1)
        var = tf.nn.softplus(raw_var) + 1e-6

        return tf.square(var-se)        
    return loss
...
def BuildModel(seqlen):
    ...
    mu_tensors=[] # keep references so var-loss can "see" mu

    for i, task in enumerate(tasks):
        task=tasks[i]
        mu_head  = kl.Dense(1, activation='linear', name=f"{task}_mu")(x)
        var_head = kl.Dense(1, activation='linear', name=f"{task}_var_raw")(x)

        outputs.extend([mu_head, var_head])
        mu_tensors.append(mu_head)

        ## Each task contributes to 2 losses: L_mu and L_var
        # L_mu = squared error on log-scale
        # L_var = (pred_var - SE)^2

        for i, task in enumerate(tasks):
            losses.append(make_mu_loss(i))      # output:{task}_mu
            losses.append(make_var_loss(i, mu_tensors[i]))  # output:{task}_var

        ## For equal weights (1,1) for both tasks 

        base_weights = [float(x) for x in config.TaskWeights]  # one per task

        for w in base_weights:
            weights.extend([w, w])  # duplicate per head
    ...
    return model, mu_tensors



def train(model,X_train,Y_train,X_valid,Y_valid):
    num_outputs = len(model.outputs)
    y_train_targets = [Y_train]*num_outputs
    y_valid_targets = [Y_valid]*num_outputs
